  ...
  return (
    <div style={{ animation:"fadeUp 0.2s ease" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:16 }}>
          {[{label:"MONEYLINE",s:stats.ml},{label:"SPREAD ATS",s:stats.spr},{label:"OVER / UNDER",s:stats.ou}].map(({label,s}) => (
            <div key={label} style={{ background:"rgba(255,200,80,0.03)", border:"1px solid rgba(255,200,80,0.13)", borderRadius:8, padding:"14px 16px" }}>
              ...
              <div style={{ fontSize:10, color:"#a8b8cc", marginTop:4 }}>{s.pct}% | ROI {parseFloat(s.roi)>=0?"+":""}{s.roi}u</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:14 }}>
        <button onClick={()=>handleFetchResults(false)} disabled={fetchingResults} style={{ background:fetchingResults?"#0f0800":"linear-gradient(135deg,#1d4ed8,#3b82f6)", border:"none", borderRadius:5, padding:"8px 16px", color:fetchingResults?"#6b7a95":"#eff6ff", fontSize:10, fontWeight:700, letterSpacing:2, fontFamily:"monospace", cursor:fetchingResults?"not-allowed":"pointer" }}>
          {fetchingResults ? "FETCHING..." : "DOWNLOAD YESTERDAY'S RESULTS"}
        </button>
        <button onClick={()=>setShowResultsPaste(v=>!v)} style={{ background:showResultsPaste?"rgba(251,191,36,0.12)":"rgba(255,200,80,0.06)", border:"1px solid rgba(255,200,80,0.2)", borderRadius:5, padding:"8px 16px", color:showResultsPaste?"#fbbf24":"#a8b8cc", fontSize:10, fontWeight:700, letterSpacing:2, fontFamily:"monospace", cursor:"pointer" }}>
          {showResultsPaste ? "HIDE" : "PASTE RESULTS CSV"}
        </button>
        <button onClick={()=>setShowPredPaste(v=>!v)} style={{ background:showPredPaste?"rgba(251,191,36,0.12)":"rgba(255,200,80,0.06)", border:"1px solid rgba(255,200,80,0.2)", borderRadius:5, padding:"8px 16px", color:showPredPaste?"#fbbf24":"#a8b8cc", fontSize:10, fontWeight:700, letterSpacing:2, fontFamily:"monospace", cursor:"pointer" }}>
      ...
      {showResultsPaste && (
        <div style={{ background:"rgba(255,200,80,0.03)", border:"1px solid rgba(255,200,80,0.15)", borderRadius:6, padding:14, marginBottom:12, animation:"fadeUp 0.2s ease" }}>
          <div style={{ fontSize:9, color:"#9aaabb", letterSpacing:3, marginBottom:8 }}>PASTE RESULTS CSV Â· columns: Date, Home, Away, Home Score, Away Score</div>
          ...
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            ...
            <button onClick={()=>{ setResultsPaste(""); setShowResultsPaste(false); }} style={{ background:"transparent", border:"1px solid rgba(255,200,80,0.15)", borderRadius:4, padding:"7px 14px", color:"#8a97b5", fontSize:10, fontFamily:"monospace", cursor:"pointer" }}>CANCEL</button>
          </div>
        </div>
      )}
      ...
      {showPredPaste && (
        <div style={{ background:"rgba(255,200,80,0.03)", border:"1px solid rgba(255,200,80,0.15)", borderRadius:6, padding:14, marginBottom:12, animation:"fadeUp 0.2s ease" }}>
          <div style={{ fontSize:9, color:"#9aaabb", letterSpacing:3, marginBottom:8 }}>PASTE PREDICTIONS CSV | export from the Predictor tab using EXPORT CSV</div>
          ...
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            ...
            <button onClick={()=>{ setPredPaste(""); setShowPredPaste(false); }} style={{ background:"transparent", border:"1px solid rgba(255,200,80,0.15)", borderRadius:4, padding:"7px 14px", color:"#8a97b5", fontSize:10, fontFamily:"monospace", cursor:"pointer" }}>CANCEL</button>
          </div>
        </div>
      )}
              ...
              <thead>
                <tr style={{ borderBottom:"1px solid rgba(255,200,80,0.2)" }}>
                  {["DATE","MATCHUP","PROJ","ACTUAL","TOTAL","V.OU","O/U REC","ML REC","ML","SPR REC","SPR","RESULT"].map(h=>(
                    <th key={h} style={{ padding:"6px 8px", textAlign:"left", fontSize:8, color:"#7a8ab2", letterSpacing:2, fontWeight:700, whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {gradedRows.map((row,i) => {
                  ...
                  const wC  = (v: boolean | null | undefined) => v===true?"#3fb950":v===false?"#f87171":"#7a8ab2";
                  return (
                    <tr key={i} style={{ borderBottom:"1px solid rgba(255,200,80,0.05)", background:i%2===0?"transparent":"rgba(255,200,80,0.015)" }}>
                      ...
                      <td style={{ padding:"6px 8px", color:"#a8b8cc", whiteSpace:"nowrap" }}>{row.hProj&&row.aProj?`${row.hProj}-${row.aProj}`:"-"}</td>
                      <td style={{ padding:"6px 8px", color:has?"#e8d5a0":"#6b7a95", fontWeight:has?700:400, whiteSpace:"nowrap" }}>{has&&resultRow?`${resultRow.hScore}-${resultRow.aScore}`:"pending"}</td>
                      <td style={{ padding:"6px 8px", color:has?wC(row.ouWin):"#7a8ab2", whiteSpace:"nowrap" }}>{has?row.actualTotal:"-"}{row.modelTotal?` (m${row.modelTotal})`:""}</td>
                      <td style={{ padding:"6px 8px", color:"#9aaabb" }}>{row.vegaOU??"-"}</td>
                      <td style={{ padding:"6px 8px", whiteSpace:"nowrap" }}>
                        <span style={{ color:has?wC(row.ouWin):"#6b7a95", fontWeight:700 }}>{row.ouRec&&row.ouRec!=="-"?row.ouRec:"PASS"}</span>
                        {has&&row.ouWin!==null&&<span style={{ fontSize:9, color:wC(row.ouWin), marginLeft:4 }}>{row.ouWin?"W":"L"}</span>}
                      </td>
                      <td style={{ padding:"6px 8px", color:"#a8b8cc", whiteSpace:"nowrap" }}>
                        {row.mlRec && row.mlRec !== "-" ? row.mlRec : "PASS"}
                      </td>
                      <td style={{ padding:"6px 8px", whiteSpace:"nowrap" }}>
                        <span style={{ color:has&&row.mlWin!==null?wC(row.mlWin):"#6b7a95", fontWeight:700 }}>{has&&row.mlWin!==null?(row.mlWin?"WIN":"LOSS"):"-"}</span>
                      ...
                      <td style={{ padding:"6px 8px", color:"#a8b8cc", whiteSpace:"nowrap", fontSize:10 }}>{row.sprRec&&row.sprRec!=="-"&&row.sprRec!=="PASS"?row.sprRec:"PASS"}</td>
                      <td style={{ padding:"6px 8px", whiteSpace:"nowrap" }}>
                        <span style={{ color:has&&row.sprWin!==null?wC(row.sprWin):"#6b7a95", fontWeight:700 }}>{has&&row.sprWin!==null?(row.sprWin?"WIN":"LOSS"):"-"}</span>
      ...
      {predLog.length===0&&resultsLog.length===0&&(
        <div style={{ background:"rgba(255,200,80,0.02)", border:"1px solid rgba(255,200,80,0.08)", borderRadius:8, padding:40, textAlign:"center" }}>
          ...
          <div style={{ fontSize:11, color:"#7a8ab2", letterSpacing:2 }}>NO DATA YET</div>
          <div style={{ fontSize:10, color:"#6b7a95", marginTop:8, lineHeight:2 }}>
